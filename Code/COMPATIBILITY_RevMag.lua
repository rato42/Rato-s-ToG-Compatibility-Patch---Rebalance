function OnMsg.RevisedMagPropsAddedToFirearms()
    ...
    --- HKG3
    REV_SetupWeapon(G3A3Green_1, "HKG3", "HKG3Magazine", "MagNormal",
                    {"MagLarge", "MagNormal"})
    REV_SetupWeapon(G3A3_1, "HKG3", "HKG3Magazine", "MagNormal",
                    {"MagLarge", "MagNormal"})

    --- STANAG
    ----- Small mags (10 round) not used for now, due to model incompatibility with vanilla guns
    REV_SetupWeapon(HK33A2_1, "AR15", "STANAGMagazine", "MagLarge",
                    {"MagLarge", "MagNormal"}) -- ,"MagSmall_GBOTOG" })
    REV_SetupWeapon(HK53_1, "AR15", "STANAGMagazine", "MagLarge",
                    {"MagLarge", "MagNormal"}) -- , "MagSmall_GBOTOG" })
    REV_SetupWeapon(TAR21_1, "AR15", "STANAGMagazine", "MagLarge",
                    {"MagLarge", "MagNormal"}) -- , "MagSmall_GBOTOG" })
    ...
    --- 9x39 
    REV_SetupWeapon(VSS_1, "VSS939", "VSS_Mag", "MagNormal",
                    {"MagNormal", "MagLarge"})
    REV_SetupWeapon(VSK94_1, "VSS939", "VSK_Mag", "MagLarge",
                    {"MagNormal", "MagLarge"})

    ---- Glock
    REV_SetupWeapon(Glock17_1, "Glock", "GlockMagazine", "MagNormal",
                    {"MagNormal", "MagLarge"})
end
...
function RatTOG_RevMag_ApplyMap(map)
    ...
    for mag_id, visual_table in pairs(map) do
        ...
        for _, visual in ipairs(visual_table) do
            ...
            if not found then table.insert(mag.Visuals, visual) end
